...
type Props = {
  ...
  keyboardGestureAreaProps?: Partial<KeyboardGestureAreaProps>
}

const KeyboardAwareScrollView: FC<Props> = ({
  children,
  keyboardGestureAreaProps,
  scrollViewProps,
}) => {
  ...
  const maybeScroll = useWorkletCallback((e: number) => {
    ...
    if (visibleRect - click.value <= BOTTOM_OFFSET) {
      ...
      scrollTo(scrollViewAnimatedRef, 0, targetScrollY, true)
    }
  }, [])
  ...
  const defaultContentContainer: ViewStyle = {
  ...
  return (
    <KeyboardGestureArea
      interpolator="ios"
      {...keyboardGestureAreaProps}
      style={[commonStyles.root, { ...keyboardGestureAreaProps }?.style]}
    >
